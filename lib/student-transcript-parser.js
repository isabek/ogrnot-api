  ...
  undergraduate.each(function (index, elem) {
    ...
    if (index % 2 == 0) {
      ...
      if (genel.indexOf("Genel") == 0) {
        ...
        semester['gpa'] = gpa ? gpa : '0';
        semester['totalCredit'] = totalCredit ? totalCredit : '0';
        semester['totalAverage'] = totalAverage ? totalAverage : '0';
      }
    }
  });
...
function parseSupplement(elem) {
  return elem.eq(3).text();
}

function isOk(index, elem) {
  return isSemester(index, elem) || isPenalty(index, elem);
}
...
function isPenalty(index, elem) {
  return cheerio(elem).text().indexOf("Bu dönem disiplin cezası aldı.") != -1;   
}
